...
PkgPath() string
返回命名类型的包路径,即唯一标识包的导入路径, 例如"encoding/base64", 如果类型是预先声明的
(string, error) 或未命名的(*T,struct{},[]int), 则包路径将是空字符串.
...
NumOut() int
返回Func类型的输出参数个数. 如果类型的Kind不是Func, panic

Out(i int) Type
返回Func类型的第i个输出参数的类型. 如果类型的Kind不是Func, panic
如果i不在[0, NumOut())之间, panic

NumIn() int
返回Func类型输入参数的个数. 如果类型的Kind不是Func, panic

In(i int) Type
返回Func类型的第i个输入参数的类型. 如果类型的Kind不是Func, panic
...
NumField() int
返回Struct类型的字段总数.  如果类型的Kind不是Struct, panic

Field(i int) StructField
Field返回Struct类型的第i个字段. 如果类型的Kind不是Struct, 则会发生panic;
...
Key() Type
返回map类型当中key的类型. 如果类型的Kind不是Map, panic


Len() int
返回Array类型的长度. 如果类型的Kind不是Array, panic
*/
